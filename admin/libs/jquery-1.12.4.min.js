...
(function (w) {
	var URL_base = "http://localhost:8080/api/v1";
	var BigNews = {
		URL_base: URL_base,
		user_login: URL_base + "/admin/user/login", //用户登录
		user_info: URL_base + "/admin/user/info", //用户信息
		user_detail: URL_base + "/admin/user/detail", //用户详情
		user_edit: URL_base + "/admin/user/edit", //用户编辑
		category_list: URL_base + "/admin/category/list", //文章类别查询
		category_add: URL_base + "/admin/category/add", //文章类别新增
		category_search: URL_base + "/admin/category/search", //文章类别搜索
		category_edit: URL_base + "/admin/category/edit", //文章类别编辑
		category_delete: URL_base + "/admin/category/delete", //文章类别删除
		article_query: URL_base + "/admin/article/query", //文章搜索
		article_publish: URL_base + "/admin/article/publish", //文章发布
		article_delete: URL_base + "/admin/article/search", //文章信息查询
		article_edit: URL_base + "/admin/article/edit", //文章编辑
		article_delete: URL_base + "/admin/article/delete", //文章删除
		data_info: URL_base + "/admin/data/info", //获取统计数据
		data_article: URL_base + "/admin/data/article", //日新增文章数量统计
		data_category: URL_base + "/admin/data/category", //各类型文章数量统计
		data_visit: URL_base + "/admin/data/visit", //日文章访问量
		comment_search: URL_base + "/admin/comment/search", //文章评论搜索
		comment_pass: URL_base + "/admin/comment/pass", //文章评论通过
		comment_reject: URL_base + "/admin/comment/reject", //文章评论不通过
		comment_delete: URL_base + "/admin/comment/delete", //文章评论删除
	};
	w.BigNews = BigNews;
})(window);
// 类alert模态框
function mod_message(title, body, btn) {
	$(".modal-title").text(title);
	$(".modal-body>p").text(body);
	$(".modal-footer .btn-danger").text(btn);
	$("#myModal").modal();
}
// 单模态框双用
function mod_category() {
	...
	if (this.id == "xinzengfenlei") {
		$modal.find("#exampleModalLabel").text("新增分类");
		$modal.find(".btn-primary").text("新增");
		$modal_inputs.val("");
	} else {
		$modal.find("#exampleModalLabel").text("编辑分类");
		$modal.find(".btn-primary").text("编辑");
		$modal_inputs.val("");
	}
	$("#exampleModal").modal();
}
$.ajaxSetup({
	...
	error: function (xhr) {
		if (xhr.status === 404) {
			...
			mod_message("温馨提示", xhr.status, "确定");
		}
		if (xhr.status === 403) {
			$("#myModal").off();
			$("#myModal").on("hidden.bs.modal", function (e) {
